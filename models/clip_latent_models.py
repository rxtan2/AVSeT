...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x, use_meanpooled_regions):    
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2] * x.shape[3]).permute(2, 0, 1).contiguous()  # NCHW -> (HW)NC
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC
        x = x + self.positional_embedding[:, None, :].to(x.dtype)  # (HW+1)NC
        
        if use_meanpooled_regions:
            x = F.linear(x, self.v_proj.weight, self.v_proj.bias)
            x = F.linear(x, self.c_proj.weight, self.c_proj.bias)
            x = x.permute(1, 0, 2)
            x = x[:, 1:]
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64, use_meanpooled_regions=False):
        super().__init__()
        ...
        self.use_meanpooled_regions = use_meanpooled_regions
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        x = self.attnpool(x, self.use_meanpooled_regions)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Enable semantic comparison with language representation on a per-patch basis
        x = self.ln_post(x)
        #x = x[:, 1:, :]
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text):
        num_imgs = image.size(1)
        
        image = image.view(len(image) * num_imgs, image.size(-3), image.size(-2), image.size(-1))
        ...
        scores = logit_scale * text_features @ image_features.t()
        scores = scores.view(len(text_features), -1, num_imgs)
        return scores
...
class LatentConceptEmbedding(nn.Module):
    def __init__(self, bs, num_learnable_embeddings, num_text_classes=13, text_dim=1024, num_regions=49):
        ...
        #self.latent_concept = torch.nn.Parameter(torch.randn(bs, num_regions, num_learnable_embeddings, 512))
        ...
        self.text_class_embeddings.weight.requires_grad = False

        self.clip_model = ClipLatentModel()
        
    def extract_latent_concept(self, text, text_features):
    
        ### hardcoded for templated prompts ###
        first = text_features[:, 0].unsqueeze(1)
        second = text_features[torch.arange(text_features.shape[0]), text.argmax(dim=-1)].unsqueeze(1)
        tmp = text_features[:, -1].unsqueeze(1).repeat(1, 77 - 2 - self.num_learnable_embeddings, 1)
        second = torch.cat((second, tmp), dim=1)
    ...
    def forward(self, text, text_features, visual_features, cat_idx): 
    
        ### hardcoded for templated prompts ###
        first = text_features[:, :1]
        second = text_features[:, 1+self.num_learnable_embeddings:]
        ...
        score = final_text_features * visual_features
        score = score.sum(-1)        
        
        logit_scale = self.clip_model.logit_scale.exp()
        score = score * logit_scale
        
        score = score.mean()
        
        # compute gt text label classification
        gt_text_labels = self.text_class_embeddings.weight / self.text_class_embeddings.weight.norm(dim=-1, keepdim=True)
        ...
        text_class_scores = torch.argmax(text_class_scores, dim=-1)
        text_class_scores = text_class_scores == cat_idx
...
class ClipLatentModel(nn.Module):
    ...
    def get_token_embeddings(self, text):
        return self.token_embedding(text)
        
    def encode_text(self, x, text):
    ...
    def forward(self, tokens, visual_features, latent_concept):

        text_features = self.encode_text(latent_concept, tokens)
