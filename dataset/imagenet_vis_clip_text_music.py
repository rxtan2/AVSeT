...
class ImageNetVitClipTextMUSICMixDataset(BaseDataset):
    def __init__(self, list_sample, opt, **kwargs):
        super(ImageNetVitClipTextMUSICMixDataset, self).__init__(
        ...
        if 'vitb32-text' not in opt.arch_frame:
    ...
    def _load_clip_frames(self, paths):
        ...
        for path in paths:
            ...
            curr_frame = self.preprocess(curr_frame).unsqueeze(0)
    ...
    def __getitem__(self, index):
        ...
        ret_dict = {'mag_mix': mag_mix, 'frames': frames, 'mags': mags, 'clip_names': clip_names, 'bbox_centers': bbox_centers, 'text': text}
        if self.split != 'train':
