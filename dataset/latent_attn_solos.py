...
class LatentAttnSOLOSMixDataset(BaseDataset):
    def __init__(self, list_sample, opt, **kwargs):
        super(LatentAttnSOLOSMixDataset, self).__init__(
        ...
        self.latent_video_features = np.load("/research/rxtan/object-detection/models/Sound-of-Pixels/precomputed_features/latent_features/solos_1_latent_concept_sgd_epoch_10000_lr_10.0_model.npy")
        latent_video_order = pickle.load(open("/research/rxtan/object-detection/models/Sound-of-Pixels/precomputed_features/latent_features/solos_1_latent_concept_sgd_epoch_10000_lr_10.0_model.pkl", 'rb'))
        
        self.vid2idx = {}
        for idx, vid in enumerate(latent_video_order):
            self.vid2idx[vid] = idx
    ...
    def __getitem__(self, index):
        ...
        vid_idx = self.vid2idx[vid]
        
        vid = vid.split('.mp4')[0]  
        cat = cat.lower()
        frames = self._load_clip_frames(path_frames)
        
        text_feat = torch.from_numpy(self.latent_video_features[vid_idx])
        ret_dict = {'text': text_feat, 'frames': frames, 'path_frames': path_frames}
