...
class LanguageMUSICMixDataset(BaseDataset):
    def __init__(self, list_sample, opt, **kwargs):
        super(LanguageMUSICMixDataset, self).__init__(
        ...
        json_files = ["./../../datasets/MUSIC_dataset/MUSIC21_solo_videos.json", "./../../datasets/MUSIC_dataset/MUSIC_duet_videos.json", "./../../datasets/MUSIC_dataset/MUSIC_solo_videos.json"]
        ...
        for cat in self.all_cats:
          cat = cat.replace(' ', ' and ')
          cat = cat.replace('_', ' ')
          cat = self.tokenizer(cat)['input_ids']
          if len(cat) > max_len:
            max_len = len(cat)
